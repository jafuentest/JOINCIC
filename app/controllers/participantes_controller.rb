  ...
  def controlDeVentas
    ...
    dias.each do |d|
      ...
      ventasDelDia = Participante.find(:all, :conditions => ["date(convert_tz(created_at,'+0:00','-4:30')) >= ? AND date(convert_tz(created_at,'+0:00','-4:30')) <= ?", fecha, fecha])
        dia[:UCAB]   = ventasDelDia.count{ |p| not p.organizador.nil? and p.organizador.institucion == "UCAB" }
        dia[:UCV]    = ventasDelDia.count{ |p| not p.organizador.nil? and p.organizador.institucion == "UCV" }
        dia[:UNEFA]  = ventasDelDia.count{ |p| not p.organizador.nil? and p.organizador.institucion == "UNEFA" }
        dia[:USB]    = ventasDelDia.count{ |p| not p.organizador.nil? and p.organizador.institucion == "USB" }
      @fechas << dia
    end
